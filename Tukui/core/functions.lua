...
T.PP = function(p, obj)
	...
	if p == 1 then
		obj:SetParent(TukuiDatapanelL4)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiDatapanelL4)
		obj:SetPoint('BOTTOM', TukuiDatapanelL4, 0,-2)
	elseif p == 2 then
		obj:SetParent(TukuiDatapanelL3)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiDatapanelL3)
		obj:SetPoint('BOTTOM', TukuiDatapanelL3, 0,-2)
	elseif p == 3 then
		obj:SetParent(TukuiDatapanelL2)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiDatapanelL2)
		obj:SetPoint('BOTTOM', TukuiDatapanelL2, 0,-2)
	elseif p == 4 then
		obj:SetParent(TukuiDatapanelL1)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiDatapanelL1)
		obj:SetPoint('BOTTOM', TukuiDatapanelL1, 0,-2)
	elseif p == 5 then
		obj:SetParent(TukuiMiddlePanel)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiMiddlePanel)
		obj:SetPoint('BOTTOM', TukuiMiddlePanel, 0,-2)
	elseif p == 6 then
		obj:SetParent(TukuiDatapanelR1)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiDatapanelR1)
		obj:SetPoint('BOTTOM', TukuiDatapanelR1, 0,-2)
	elseif p == 7 then
		obj:SetParent(TukuiDatapanelR2)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiDatapanelR2)
		obj:SetPoint('BOTTOM', TukuiDatapanelR2, 0,-2)
	elseif p == 8 then
		obj:SetParent(TukuiDatapanelR3)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiDatapanelR3)
		obj:SetPoint('BOTTOM', TukuiDatapanelR3, 0,-2)
	elseif p == 9 then
		obj:SetParent(TukuiDatapanelR4)
		obj:SetHeight(pheight)
		obj:SetPoint('TOP', TukuiDatapanelR4)
		obj:SetPoint('BOTTOM', TukuiDatapanelR4, 0,-2)
	end
end
...
T.PostUpdateHealth = function(health, unit, min, max)
	if not UnitIsConnected(unit) or UnitIsDead(unit) or UnitIsGhost(unit) then
		...
		if min ~= max then
			...
			if (unit == "player" and health:GetAttribute("normalUnit") ~= "pet") or unit == "target" or unit == "focus"  or unit == "focustarget" or (unit and unit:find("arena%d")) then
...
T.PostUpdatePower = function(power, unit, min, max)
	...
	elseif UnitIsDead(unit) or UnitIsGhost(unit) then
		...
		if min ~= max then
			if pType == 0 then
				if unit == "target" or unit == "player" or (unit and unit:find("arena%d")) then
					if C["unitframes"].showtotalhpmp == true then
			...
			if (unit and unit:find("boss%d")) or unit == "focus" or unit == "focustarget" then
				power.value:SetText("100%")
			else
		...
		if self.elapsed >= 0.1 then
			...
			if self.timeLeft > 0 then
				...
				if self.timeLeft <= 10 then
					self.remaining:SetTextColor(1,.18,.18)
				else
...
T.PostCreateAura = function(element, button)
	...
	button.Glow:Point("TOPLEFT", button, "TOPLEFT", -1, 1)
	button.Glow:Point("BOTTOMRIGHT", button, "BOTTOMRIGHT", 1, -1)
	button.Glow:SetFrameStrata("BACKGROUND")	
	button.Glow:SetBackdrop{edgeFile = C["media"].glowTex, edgeSize = 1, insets = {left = 0, right = 0, top = 0, bottom = 0}}
...
T.ComboPointDisplay = function (self, login)
...
T.UpdateThreat = function(self, event, unit)
	...
	if threat and threat > 1 then
			self.AggroInd:SetBackdropBorderColor(1,.18,.13,1)
			self.AggroInd:SetAlpha(1)
		else
			self.AggroInd:SetBackdropBorderColor(0,0,0,0)
			self.AggroInd:SetAlpha(0)
		end 
end
