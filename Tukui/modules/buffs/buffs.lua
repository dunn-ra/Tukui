	...
	if(self.expiration) then	
		...
		if(self.expiration <= 0) then
			...
			elseif self.expiration <= 60.5 and self.expiration > 10.5 then
				self.time:SetText("|cffFF3030"..time.."|r")
...
local function UpdateAuras(header, button, weapon)
	...
	if(not button.texture) then
		...
		button.count:SetFont(C.media.font, 11, "OUTLINE")
...
ScanAuras(CreateAuraHeader("HELPFUL", "TOPRIGHT", -145, -7))
ScanAuras(CreateAuraHeader("HARMFUL", "TOPRIGHT", -145, -107))
