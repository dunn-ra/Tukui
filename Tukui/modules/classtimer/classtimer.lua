...
local CAST_SEPARATOR = db.castseparator
...
local MASTER_FONT = {C["media"].uffont, 11, "OUTLINE"}
local TIME_FONT = {C["media"].uffont, 10, "OUTLINE"}
local STACKS_FONT = {C["media"].uffont, 9, "OUTLINE"}
local pwrheight = C["ufsizes"].unitheight/4
...
local TRINKET_FILTER = {
	...
	CreateSpellEntry( 91322 ), -- Blind Spot (Jar of Ancient Remedies)
	CreateSpellEntry( 92331 ), -- Blind Spot (H Jar of Ancient Remedies)
	...
	CreateSpellEntry( 90159 ), -- Druid Resto
	CreateSpellEntry( 90163 ), -- Druid Moonkin
	CreateSpellEntry( 90165 ), -- Druid Feral
	CreateSpellEntry( 90311 ), -- Paladin Holy
	CreateSpellEntry( 89911 ), -- Priest Holy/Disc
	CreateSpellEntry( 90472 ), -- Rogue
	CreateSpellEntry( 90498 ), -- Shaman Resto
	CreateSpellEntry( 89937 ), -- Warlock
	CreateSpellEntry( 90294 ), -- Warrior DPS
...
local function OnUnitFramesLoad(self, event, addon)
	if not (addon == "Tukui_Raid" or addon == "Tukui_Raid_Healing") then return end
	...
	do
		...
		CreateFramedTexture = function(parent)
			...
			background:SetPoint("TOPLEFT", border, "TOPLEFT", 0, 0)
			background:SetPoint("BOTTOMRIGHT", border, "BOTTOMRIGHT", 0, 0)
			texture:SetPoint("TOPLEFT", background, "TOPLEFT", 0, 0)
			texture:SetPoint("BOTTOMRIGHT", background, "BOTTOMRIGHT", 0, 0)
	...
	do
		...
		do
			...
			CreateAuraBar = function(parent)
				...
				icon:SetPoint("TOPLEFT", result, "TOPLEFT", -1, 0)
				...
				time:SetPoint("BOTTOMRIGHT", bar, "BOTTOMRIGHT", -TEXT_MARGIN*2, 1)
				result.time = time
		...
		CreateAuraBarFrame = function(dataSource, parent)
			...
			background:Point( "TOPLEFT", result, "TOPLEFT", BAR_HEIGHT+1, 0);
			background:Point( "BOTTOMRIGHT", result, "BOTTOMRIGHT", 1, 0);
			background:SetBackdrop({
				bgFile = C["media"].blank, edgeSize = 1,
				insets = {left = 0, right = 0, top = 0, bottom = 0}
			})
			background:SetBackdropColor(unpack(C["media"].ctbackdropcolor))
			result.background = background
	...
	if ( playerClass == "DEATHKNIGHT" or playerClass == "SHAMAN" or playerClass == "PALADIN" or playerClass == "DRUID" or playerClass == "WARLOCK" or playerClass == "ROGUE") then
		playerFrame:SetPoint("BOTTOMLEFT", TukuiPlayer, "TOPLEFT", 0, yOffset+pwrheight+1)
		playerFrame:SetPoint("BOTTOMRIGHT", TukuiPlayer, "TOPRIGHT", 0, yOffset+pwrheight+1)
	else
