...
function Stuffing:CreateBagFrame(w)
	...
	if w == "Bank" then
		f:Point("BOTTOMLEFT", TukuiViewport, "TOPLEFT", 5, 5)
	else
		f:Point("BOTTOMRIGHT", TukuiViewport, "TOPRIGHT", -5, 5)
	end
	...
	-- Create Buy Bags Button -- credits Elv22
	if w == "Bank" then
		f.b_purchase = CreateFrame("Button", "Stuffing_PurchaseButton"..w, f)
		f.b_purchase:Size(70, 20)
		f.b_purchase:Point("TOPLEFT", 12, -5)
		f.b_purchase:RegisterForClicks("AnyUp")
		f.b_purchase:SetTemplate("Transparent", true)
...
function Stuffing:Layout(lb)
	...
	if lb then
		...
		f.gold:SetText(GetMoneyString(GetMoney(), 11))
		f.editbox:SetFont(BAGSFONT, 11)
		f.detail:SetFont(BAGSFONT, 11)
	...
	f:Width(cols * 28 + (cols - 1) * 4 + 12 * 2)
	f:Height(rows * 28 + (rows - 1) * 4 + off + 12 * 2)
	...
	for _, i in ipairs(bs) do
		...
		if bag_cnt > 0 then
			...
			for j = 1, bag_cnt do
				...
				if bagType == ST_SPECIAL then b.frame:SetBackdropBorderColor(.59,.98,.59) b.frame.lock = true end
