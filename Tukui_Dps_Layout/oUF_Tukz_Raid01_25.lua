...
local function Shared(self, unit)
	...
	name:SetPoint("LEFT", self, "LEFT", TukuiDB.Scale(12), 0)
	...
	if TukuiCF["unitframes"].showsymbols == true then
		...
		RaidIcon:SetHeight(TukuiDB.Scale(13*TukuiDB.raidscale))
		RaidIcon:SetWidth(TukuiDB.Scale(13*TukuiDB.raidscale))
	...
	LFDRole:SetPoint("LEFT", TukuiDB.Scale(2), TukuiDB.Scale(-1))
...
oUF:Factory(function(self)
	...
	local raid = self:SpawnHeader("oUF_TukuiDpsRaid05101525", nil, "custom [@raid26,exists] hide;show", 
		...
		'initial-width', TukuiDB.Scale(85*TukuiDB.raidscale),
		'initial-height', TukuiDB.Scale(19*TukuiDB.raidscale),	
		"showParty", true,
		...
		"groupBy", "GROUP",
		"yOffset", TukuiDB.Scale(-5)
	)
	raid:SetPoint('TOPLEFT', UIParent, 10, -300*TukuiDB.raidscale)
	...
	RaidMove:SetScript("OnEvent", function(self)
		if InCombatLockdown() then
			...
			if numparty > 0 and numraid == 0 or numraid > 0 and numraid <= 5 then
				raid:SetPoint("TOPLEFT", UIParent, "TOPLEFT", 10, -399*TukuiDB.raidscale)
				for i,v in ipairs(pets) do v:Disable() end
			elseif numraid > 5 and numraid < 11 then
				raid:SetPoint('TOPLEFT', UIParent, 10, -300*TukuiDB.raidscale)
				for i,v in ipairs(pets) do v:Disable() end
			elseif numraid > 10 and numraid < 16 then
				raid:SetPoint('TOPLEFT', UIParent, 10, -230*TukuiDB.raidscale)
				for i,v in ipairs(pets) do v:Disable() end
			elseif numraid > 15 and numraid < 26 then
				raid:SetPoint('TOPLEFT', UIParent, 10, -150*TukuiDB.raidscale)
				for i,v in ipairs(pets) do v:Disable() end
